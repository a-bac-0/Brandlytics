...
@router.post("/process-url", response_model=VideoProcessingResponse)
async def process_video_from_url(
    ...
    """
    Process a video from URL for brand detection.
    Supports YouTube, Google Drive, and other video URLs.
    """
    ...
    try:
        ...
        if not video_name:
            ...
            video_name = f"url_video_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        # Create temporary file for download
        temp_dir = tempfile.mkdtemp()
        temp_video_path = os.path.join(temp_dir, f"{video_name}.%(ext)s")
        
        # Use yt-dlp in a subprocess with explicit Python path to avoid version conflicts
        ...
        if result.returncode != 0:
            ...
            raise HTTPException(
                status_code=400, 
                detail=f"Failed to download video from URL: {result.stderr}"
            )
        
        # Find the downloaded file
        import glob
        downloaded_files = glob.glob(os.path.join(temp_dir, f"{video_name}.*"))
        ...
        final_path = UPLOAD_DIR / f"{video_name}{Path(downloaded_path).suffix}"
        import shutil
        ...
        logger.info(f"Video downloaded successfully: {final_path}")
        
        # Process the video using the existing video detection service
        result = await video_detection_service.process_video_file(
        ...
        raise HTTPException(
            status_code=408,
            detail="Video download timed out. Please try with a shorter video or upload the file directly."
        )
    except ImportError as e:
        raise HTTPException(
            status_code=503,
            detail="yt-dlp not available. Please install it or upload the video file directly."
        )
    except Exception as e:
        logger.error(f"Error processing video from URL {url}: {str(e)}")
        raise HTTPException(
            status_code=500,
